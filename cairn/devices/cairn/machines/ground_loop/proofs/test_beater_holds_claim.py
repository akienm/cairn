...
Teeth a hollow build could not pass:
  ...
  - THE BEATER PID IS READ FROM LIVENESS, NOT FROM os.getpid(). Verified by constructing
    ...
    python3 cairn/devices/cairn/machines/ground_loop/proofs/test_beater_holds_claim.py   # exit 0 = green
"""
...
from cairn.devices.cairn.machines.ground_loop.probes.does_the_beater_hold_the_claim import (
    check, lock_holder_pid, beater_pid,
)
from cairn.devices.cairn.machines.ground_loop.guard import LOCK_NAME
from cairn.devices.cairn.machines.ground_loop.liveness import RECORD_NAME
