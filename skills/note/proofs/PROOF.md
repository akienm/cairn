...
- `/note` writes a record that conforms to `CairnCommons/notes/_charter+why.json`.
- The record is a capture only — it is not cast, not proved, binds no gates.
- Ceremony is minimal: text is the only required input.
