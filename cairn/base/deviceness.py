...
THIS COMPOSES, IT DOES NOT RE-DERIVE. Membership is ``cairn.ground_loop.discovery``'s to
...
HEALTH_QUERY_CLAUSE = (
    ...
    "line of UU ancestry in cairn/base/device.py. is_device() therefore answers the "
...
def fitted_device_ids(root: Path | str | None = None) -> set[str]:
    ...
    Composed from ``cairn.ground_loop.discovery.device_folders``, never reimplemented: the
    ...
    """
    from cairn.ground_loop.discovery import device_folders  # deferred: ground_loop imports base
...
def claims_device_by_inheritance(root: Path | str | None = None) -> set[str]:
    ...
    """
    from cairn.ground_loop.discovery import repo_root  # deferred, same reason as above
