...
not work yet.
    ...
    python3 cairn/db_domain/proofs/test_sole_path.py     # exit 0 = green
"""
...
def test_db_domain_is_the_only_module_that_can_reach_5432():
    ...
    assert caught == [], (
        ...
        "importer is legitimate, it belongs inside cairn/db_domain/ or in this rule WITH a "
...
def test_the_tooth_reds_on_a_planted_second_door():
    ...
    planted["cairn/librarian/store.py"] = sieve.imports_in("import psycopg2")
    planted["cairn/db_domain/store.py"] = sieve.imports_in("import psycopg2")
    caught = sieve.catches(planted, RULE)
    assert len(caught) == 1, f"exactly the rogue, and db_domain itself spared: {caught}"
    assert "cairn/librarian/store.py" in caught[0]
