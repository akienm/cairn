"""PROBE — does a message sent to CodeMonkey reach the shim's deliver()?

Berthed beside cairn/devices/codemonkey because that is WHAT IT WATCHES:
the shim's deliver() path is the one way bus messages reach CodeMonkey.
...
PROBE = Probe(
    why="the CodeMonkey device's shim must receive bus messages — without delivery, "
