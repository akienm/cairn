...
piece of code. Three destinations, chosen by the subject:
  ...
  WATCHER — it watches whether the WATCHME machinery in ``cairn/base`` actually fires, i.e.
  it watches the TOOLS — so it berths at ``cairn/base/probes/``, checked in, nowhere near
...
posting — see ``cairn/base/shim.py``). Because a probe holds no state, its firing can be
