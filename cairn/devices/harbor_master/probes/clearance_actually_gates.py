...
(object ``clearance-actually-gates``). Berthed beside ``cairn/devices/harbor_master`` because
...
berths in ``cairn/tools/base/transitions.py``; the *authority* being watched is here.
...
A COUNTED FIXTURE REFUSAL WOULD BE A LIE. ``cairn/tools/base/proofs/test_transitions.py``
...
the three siblings at ``cairn/tools/base/probes/``.
...
from cairn.devices.harbor_master.clearance import GRANTED, read_attempts
...
def _is_summons(state: str) -> bool:
    ...
    from cairn.tools.base.transitions import is_summons
...
# cairn/tools/base/probes/: the beat rate is not yet a real number, so 1000 pulses is "clearly
