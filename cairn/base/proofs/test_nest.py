...
Teeth a hollow lift could not pass:
  - THE TOY SECOND TENANT (the load-bearing row, ticket banding-berths-at-the-
    ...
    cairn.base.nest. If the "general" berth needed tenant internals — the census, the
...
Runnable bare (no DB, no framework):
    python3 cairn/base/proofs/test_nest.py     # exit 0 = green
"""
...
from cairn.base.nest import BAND_NAMES, nest, shake
...
def _main() -> int:
    ...
    print("green — nest: the assembly is coarsest-first with empty bands omitted, the "
          ...
          "importing ONLY cairn.base.nest, and the roll-up is min() — one zero sinks "
          "the subject")
    return 0
