...
Teeth a hollow build could not pass:
  ...
  - DISCOVERY ARMS IT WITH NO HAND-REGISTRATION, and imports clean. ``cairn/base`` is itself
...
Runnable bare (no DB, no framework, no network):
    python3 cairn/base/proofs/test_deviceness.py     # exit 0 = green
"""
...
from cairn.base.deviceness import (
    HEALTH_QUERY_CLAUSE,
...
from cairn.ground_loop.discovery import device_folders
...
def test_the_unbuilt_half_is_declared_not_omitted():
    ...
    from cairn.base.probes.device_claims_match_shims import PROBE
...
def test_the_enough_condition_is_reachable():
    ...
    from cairn.base.probes import device_claims_match_shims as m
...
def test_discovery_arms_the_probe_with_no_hand_registration():
    """``cairn/base`` is itself a discovered device, so the pass that reports the divergence
    is the pass that arms the reporter. No subscribe call, no list to go stale."""
    from cairn.ground_loop.discovery import discover

    found = discover()
    assert "base" in found, "cairn/base must be discovered as a device"
