"""Proof for cairn/machines/ruling — the ruling intake gate.
...
def test_the_hook_goes_quiet_after_a_cli_supersession():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        run = lambda *args: subprocess.run(
            [sys.executable, "-m", "cairn.machines.ruling.cli", *args], input="{}",
...
def test_the_hook_names_what_is_open_and_never_wedges_a_turn():
    with tempfile.TemporaryDirectory() as d:
        ...
        run = lambda: subprocess.run(
            [sys.executable, "-m", "cairn.machines.ruling.cli", "--hook"], input="{}",
            capture_output=True, text=True, cwd=str(_REPO_ROOT), env=env)
...
def _corpus_world(d: str) -> str:
    ...
    """
    _world(d)
    pkg = Path(d, "cairn", "cairn", "tools", "base")
    pkg.mkdir(parents=True, exist_ok=True)
    (pkg / "probe.py").write_text("class Probe:\n    pass\n")
    (pkg / "lonely.py").write_text("# nothing imports me\n")
    (pkg / "shim.py").write_text("from cairn.tools.base.probe import Probe\n")
    for i in range(20):
...
def test_a_file_the_corpus_still_imports_cannot_be_sentenced():
    ...
    with tempfile.TemporaryDirectory() as d:
        _corpus_world(d)
        bad = ruling.refusals(_packet(what_dies=["cairn/cairn/tools/base/probe.py"],
                                      what_conforms=[]), d)
...
def test_a_file_nothing_imports_may_still_be_sentenced():
    ...
    with tempfile.TemporaryDirectory() as d:
        _corpus_world(d)
        bad = ruling.refusals(_packet(what_dies=["cairn/cairn/tools/base/lonely.py"],
                                      what_conforms=[]), d)
