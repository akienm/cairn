...
DB teeth need the one-time provisioning (as the tree proof). Self-cleaning.
    ...
    python3 cairn/devices/codemother/machines/triage/proofs/test_chart_triage.py     # exit 0 = green
"""
...
import cairn.devices.codemother.machines.decompose.decompose as decompose_mod
from cairn.devices.codemother.machines.triage import triage as triage_mod
from cairn.devices.codemother.machines.triage.triage import (
...
def _nexus() -> str:
    ...
    if not _HELD:
        _HELD.append(_SCRATCH.enter_context(scratch_nexus("triage")))
    return _HELD[0]
...
def test_the_door_composes_the_installed_judges(root, survey_berth,
    ...
    assert "cairn.devices.codemother.machines.triage.triage" not in inspector_src, \
...
def test_import_allowlist(root, survey_berth, decompose_berth):
    ...
    allow = ("__future__", "hashlib", "json", "os", "time",
             "cairn.machines.build_inspector.inspector", "cairn.devices.codemother.machines.decompose.decompose",
