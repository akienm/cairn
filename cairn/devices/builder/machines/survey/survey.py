...
JUDGE_REFUSAL_SURVEY = (
    "survey packet refused by the installed judges (the door and the promotion gate are one implementation): ")


def inspect_survey(packet: dict, root: str = CAIRN_ROOT) -> list:
    """SURVEY'S OWN INSPECTOR — the proof record for the packet it hands the next stage.
    ...
    Takes no verdict — that is ``validate_survey``'s, at this same address, because the
    ...
    when the packet carries a constrain_ref, and the judges only once shape holds (a judge
    ...
    if "constrain_ref" in packet:
        try:
            ...
            record.append(inspected(
                "upstream_berth_is_readable", stage="survey",
            ...
            record.append(inspected(
                "request_identity_rides_the_chain", stage="survey",
                expected="consistent",
    ...
    record += common_shape_record(packet, required_fields=REQUIRED_FIELDS,
                                  ...
                                  list_fields=('holdings', 'absences', 'unknowns'),
                                  root=root, stage="survey")
...
def validate_survey(packet: dict, root: str = CAIRN_ROOT) -> dict:
    """SURVEY'S OWN GATE at the handoff — an == compare over ``inspect_survey``'s record.
