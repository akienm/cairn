...
DB teeth need the one-time provisioning (as the tree proof). Self-cleaning.
    ...
    python3 cairn/chart/proofs/test_chart_hypothesize.py     # exit 0 = green
"""
...
import cairn.chart.triage as triage_mod
from cairn.chart import hypothesize as hypothesize_mod
from cairn.chart.hypothesize import (
...
def make_root():
    ...
    pieces."""
    tmp = tempfile.mkdtemp(prefix="chart_hypothesize_proof_")
...
def test_the_door_composes_the_installed_judges(root, decompose_berth,
    ...
    assert "cairn.chart.hypothesize" not in inspector_src, \
...
def test_import_allowlist(root, decompose_berth, triage_berth):
    ...
    allow = ("__future__", "hashlib", "json", "os", "time",
             ...
             "cairn.chart.tree", "cairn.chart.triage")
