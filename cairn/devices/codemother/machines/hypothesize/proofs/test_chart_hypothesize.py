...
DB teeth need the one-time provisioning (as the tree proof). Self-cleaning.
    ...
    python3 cairn/devices/codemother/machines/hypothesize/proofs/test_chart_hypothesize.py     # exit 0 = green
"""
...
import cairn.devices.codemother.machines.triage.triage as triage_mod
from cairn.devices.codemother.machines.hypothesize import hypothesize as hypothesize_mod
from cairn.devices.codemother.machines.hypothesize.hypothesize import (
...
def _nexus() -> str:
    ...
    if not _HELD:
        _HELD.append(_SCRATCH.enter_context(scratch_nexus("hypothesize")))
    return _HELD[0]
...
def test_the_door_composes_the_installed_judges(root, decompose_berth,
    ...
    assert "cairn.devices.codemother.machines.hypothesize.hypothesize" not in inspector_src, \
...
def test_import_allowlist(root, decompose_berth, triage_berth):
    ...
    allow = ("__future__", "hashlib", "json", "os", "time",
             ...
             "cairn.tools.tree.tree", "cairn.devices.codemother.machines.triage.triage",
