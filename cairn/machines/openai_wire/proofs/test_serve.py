...
# end through the real handler. The REAL holder is ticket cb97524c0e8e's; the live fire in
# this voyage's verdict artifact is where a real model answered through make_server's door.
PROVES = {
    "76639374d9f9": {
        ...
        "e": "a_holder_serves_a_tool_using_turn_end_to_end",
    },
}
...
def a_holder_serves_a_tool_using_turn_end_to_end():
    ...
    gets the final text. Both turns are real HTTP bytes parsed by the real handler class and
    read back off the wire it wrote; the resolve sees provider-shaped arguments (objects) only
    because the holder ran translate.to_provider, which is the holder's job.

    Why no listener here: the sole-path sieve (build_inspector.sole_path_holds) does not
    exempt proofs, and every client that could dial a loopback port is on its list — a proof
    of an HTTP face cannot bind and dial without becoming a second door. So the TCP listener
    is exercised where a dial is allowed to live: the voyage's verdict instrument
    (CairnCommons/measurements/2026-09-12-76639374d9f9-verdict-instruments.py live_two_turn)
    runs these same two turns through make_server against the real inference host."""
    turns = []
    ...
    handler = _handler(resolve=bus_ask, models=lambda: ["m"])
    tools = [{"type": "function", "function": {"name": "read_file",
                                               "parameters": {"type": "object"}}}]
    history = [{"role": "user", "content": "what is in a.py?"}]
    status, first = _drive(handler, "POST", "/v1/chat/completions",
    ...
    status, second = _drive(handler, "POST", "/v1/chat/completions",
                            {"model": "m", "messages": history, "tools": tools})
    ...
    status, listed = _drive(handler, "GET", "/v1/models")
    assert status == 200 and [d["id"] for d in listed["data"]] == ["m"], listed
...
TEETH = [
    ...
    a_holder_serves_a_tool_using_turn_end_to_end,
