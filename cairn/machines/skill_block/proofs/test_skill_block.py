"""Proof for cairn/machines/skill_block — teeth a hollow seam could not pass.
...
one that passes everything fails the refusal half.
    ...
    python3 cairn/machines/skill_block/proofs/test_skill_block.py     # exit 0 = green
"""
...
def _cli(*args, cwd=REPO):
    ...
    return subprocess.run([sys.executable, "-m", "cairn.machines.skill_block", *args],
...
def test_the_wire_is_not_fatal_when_the_ROOTS_are_unwritable():
    ...
    try:
        ...
        packet.write_text(json.dumps(LIVE_GOOD))
        r = subprocess.run(
            [sys.executable, "-m", "cairn.machines.skill_block", "fire", "intent", str(packet)],
...
def test_the_seam_runs_the_skills_own_judge():
    ...
    `python3 skills/<name>/door.py`, so `python3 -m cairn.machines.skill_block fire <name>` — the
