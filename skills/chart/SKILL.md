...
import json, sys
from cairn.machines.orient.orient import floor_facts
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>'
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.orient.orient import write_packet
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the berth path>
```
...
conversation. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.constrain.constrain import constrain_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' constrain
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.constrain.constrain import write_constrain
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the constrain berth path>
```
...
Carry the constrain berth path — stage 3 template-fills from it. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.survey.survey import survey_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' survey
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.survey.survey import write_survey
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the survey berth path>
```
...
Carry the survey berth path — stage 4 template-fills from it. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.decompose.decompose import decompose_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' decompose
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.decompose.decompose import write_decompose
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the decompose berth path>
```
...
Carry the decompose berth path — stage 5 template-fills from it. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.triage.triage import triage_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' triage
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.triage.triage import write_triage
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the triage berth path>
```
...
Carry the triage berth path — stage 6 template-fills from it. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.hypothesize.hypothesize import hypothesize_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' hypothesize
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.hypothesize.hypothesize import write_hypothesize
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the hypothesize berth path>
```
...
Carry the hypothesize berth path — stage 7 template-fills from it. Lost after a compact? `python3 -m skills.chart.live chain <ticket>` lists the standing berths for a claiming chain.
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.validate.validate import validate_floor
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live counsel '<the request, verbatim>' validate
```
...
PYTHONPATH=$HOME/dev/src/cairn python3 -c "
import json, sys
from cairn.machines.validate.validate import write_validate
...
```bash
PYTHONPATH=$HOME/dev/src/cairn python3 -m skills.chart.live learn <the validate berth path>
```
...
**The chain is complete**: report the seven berths — and the resolver can re-list them any time: `python3 -m skills.chart.live chain <ticket>` (ticket berths-carry-request-identity). The charted course —
