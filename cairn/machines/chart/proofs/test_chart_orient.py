...
from cairn.machines.chart.orient import (AUTHORED_FIELDS, OrientRefused, component_roster,
...
# cairn.tools.base.address joined 2026-08-12 (one-owner-for-the-instance-address). This tooth's
...
def test_the_roster_is_measured_ONCE_per_process_and_forgetting_re_measures(root):
    ...
    line. The lived symptom was ``cairn/machines/build_inspector``: the only proof in the corpus
    ...
    identical defect."""
    import cairn.machines.chart.orient as orient_mod
    calls = []
...
def test_import_allowlist(root):
    """Stdlib plus EXACTLY ONE door into the house: cairn.tools.orient.orient (the
...
def test_request_identity_helper(root):
    ...
    from cairn.machines.chart.orient import identity_lack
