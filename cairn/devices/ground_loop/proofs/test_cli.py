"""Proofs for cairn/devices/ground_loop/cli.py — the operator's interface.
...
from cairn.devices.ground_loop.cli import _status, _stop, _start, _clear_trouble, main
from cairn.devices.ground_loop.liveness import write_liveness
...
def test_status_reads_liveness():
    ...
    d = _tmp_instance()
    try:
        with patch("cairn.devices.ground_loop.cli._home", return_value=Path(d)):
            rc = _status()
        if rc == 0:
            PASS += 1
...
def test_dispatcher_exists_and_is_executable():
    global PASS, FAIL
    # proofs/ -> ground_loop/ -> devices/ -> cairn/ -> repo root
    script = Path(__file__).resolve().parent.parent.parent.parent.parent / "bin" / "cmd" / "groundloop"
