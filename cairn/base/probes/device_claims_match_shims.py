...
here, beside ``cairn/base``, for two reasons that happen to coincide: this is WHAT IT
WATCHES (``BaseDevice`` lives one directory up and is the artifact whose claim this
question is about), and ``cairn/base`` is ITSELF a discovered device — it holds this
...
the same day (``cairn/ground_loop/discovery.py``, commit 2081efa). So the mechanism is not
...
``ground_loop`` — the spine — are not devices under the ruling, and that ``cairn/base``
...
cairn.ground_loop`` is running (pid 109150, up since 13:37:45), 1344 beats, pulsing 13
...
from cairn.base.deviceness import HEALTH_QUERY_CLAUSE, divergence
...
def measure(context: dict | None = None) -> dict:
    ...
    Composed from ``cairn.base.deviceness`` rather than re-derived here. A probe that
...
# question ``cairn/base``'s charter owes and nothing here pretends to answer.
_HORIZON = 100
