"""Proof: the RULES STACK routes (cairn/inference_domain/route.py + stacks/), or refuses loudly.
...
live measurement against real Hex is the validation's separate, deliberate act.
    ...
    python3 cairn/inference_domain/proofs/test_route.py     # exit 0 = green, no host, no ~/.cairn
"""
...
from cairn.inference_domain import host, route
...
def test_the_nest_is_the_general_nest_not_a_private_copy():
    """The route is the SECOND CARRIER of cairn.base.nest (build_inspector is the first) — read
    ...
    carried = any(
        (isinstance(n, ast.ImportFrom) and n.module == "cairn.base"
         and any(a.name == "nest" for a in n.names))
        or (isinstance(n, ast.ImportFrom) and n.module == "cairn.base.nest")
        or (isinstance(n, ast.Import) and any(a.name == "cairn.base.nest" for a in n.names))
        for n in ast.walk(tree))
    assert carried, "route.py must IMPORT cairn.base.nest — a private shake would be the reified copy"
