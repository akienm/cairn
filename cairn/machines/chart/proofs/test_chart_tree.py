...
the Unix socket is a file (db_domain's documented asymmetry).
    ...
    python3 cairn/machines/chart/proofs/test_chart_tree.py     # exit 0 = green
"""
...
from cairn.machines.chart import dial as dial_mod
from cairn.machines.chart import tree
from cairn.machines.chart.dial import dial
from cairn.machines.chart.orient import STRATA, OrientRefused
from cairn.machines.chart.tree import (
    TreeRefused, counsel, deposit_learning, deposit_packet, nexus_table,
)
...
def test_gate_before_seed():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _refuses(OrientRefused, deposit_packet, broken, [1.0, 0.0, 0.0], berth_path=berth)
        # A berth that does not exist on disk refuses — provenance may not point at nothing.
        msg = _refuses(TreeRefused, deposit_packet, _packet(), [1.0, 0.0, 0.0],
...
def test_deposit_back_lands_and_dedups():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = deposit_packet(packet, [0.9, 0.1, 0.0], berth_path=berth, nexus=_NEXUS)
        assert r["duplicate"] is False
        ...
        before = trees.tree_state(_NEXUS, table=_TABLE, owner="chart")
        r2 = deposit_packet(packet, [0.9, 0.1, 0.0], berth_path=berth, nexus=_NEXUS)
...
def test_the_librarians_tools_are_the_only_door():
    allowed = {
        tree.__file__: ("__future__", "os", "re", "cairn.machines.chart.orient",
                        "cairn.devices.librarian.trees", "cairn.devices.librarian.loop"),
        # cairn.machines.chart.constrain joined 2026-07-28 (chart-constrain), cairn.machines.chart.survey
        # and cairn.machines.chart.decompose the same day (chart-survey, chart-decompose),
        # cairn.machines.chart.triage the same day again (chart-triage), then
        # cairn.machines.chart.hypothesize and cairn.machines.chart.validate (chart-hypothesize,
        ...
        dial_mod.__file__: ("__future__", "json", "os", "re", "sys",
                            "cairn.machines.chart.constrain", "cairn.machines.chart.decompose",
                            "cairn.machines.chart.hypothesize", "cairn.machines.chart.orient",
                            "cairn.machines.chart.survey", "cairn.machines.chart.triage",
                            "cairn.machines.chart.validate"),
    }
...
def test_tree_is_a_legal_stratum_at_the_packet_gate():
    from cairn.machines.chart.orient import validate_orient
