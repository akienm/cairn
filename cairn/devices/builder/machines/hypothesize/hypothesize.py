...
from cairn.tools.chain.grammar import (CAIRN_ROOT, INSTANCE_DIR, STRATA, ticket_claim_error, common_shape_lacks, render_lacks, CHAIN_REMEDY, identity_lack)
from cairn.tools.tree.tree import deposit_learning
from cairn.devices.builder.machines.triage.triage import _read_decompose_berth
...
def validate_hypothesize(packet: dict, root: str = CAIRN_ROOT) -> dict:
    ...
    if verdicts:
        raise HypothesizeRefused(
            "hypothesize packet refused by the installed judges (the door and "
            "the promotion gate are one implementation): "
