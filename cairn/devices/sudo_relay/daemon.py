...
Start it:   python3 cairn/devices/sudo_relay/daemon.py
Watch it:   python3 cairn/devices/sudo_relay/daemon.py --status
...
# Run directly as a script (Akien: `python3 cairn/devices/sudo_relay/daemon.py`) — put the repo
# root on the path so `cairn` imports. daemon.py is at cairn/devices/sudo_relay/daemon.py, so the
