...
but ``cairn/tools/base/probes/does_optional_mean_never_carried.py`` is the REAL berth, loaded from
...
THE TWO LOAD-BEARING ROWS:
  ...
  - ``test_a_refusal_journals_absolutely_nothing`` — a refused move must leave no partial
    ...
    python3 cairn/tools/base/proofs/test_emission_gate.py     # exit 0 = green
"""
...
from cairn.tools.base import transitions, watchme_spec
from cairn.tools.charter import projector

_CD = transitions.load_class_def("code-seam")
_REAL_BERTH = "cairn/tools/base/probes/does_optional_mean_never_carried.py"
...
def _cleared(comp: Path) -> dict:
    ...
    persist_validation({
        ...
        "caller": "cairn/tools/base/proofs/test_emission_gate.py",
...
def test_a_promised_probe_the_world_does_not_hold_is_refused():
    spec = _spec(probe="cairn/tools/base/probes/a_probe_nobody_wrote.py")
