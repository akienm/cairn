...
DB teeth need the one-time provisioning (as the tree proof). Self-cleaning.
    ...
    python3 cairn/devices/codemother/machines/validate/proofs/test_chart_validate.py     # exit 0 = green
"""
...
import cairn.devices.codemother.machines.hypothesize.hypothesize as hypothesize_mod
from cairn.devices.codemother.machines.validate import validate as validate_mod
from cairn.devices.codemother.machines.validate.validate import (
...
def test_the_door_composes_the_installed_judges(root, triage_berth,
    ...
    assert "cairn.devices.codemother.machines.validate.validate" not in inspector_src, \
...
def test_import_allowlist(root, triage_berth, hypothesize_berth):
    ...
    allow = ("__future__", "hashlib", "json", "os", "time",
             "cairn.machines.build_inspector.inspector", "cairn.devices.codemother.machines.hypothesize.hypothesize",
...
def test_refusal_is_one_pass_complete(root, triage_berth, hypothesize_berth):
    ...
    try:
        validate_floor(os.path.join(root, "no-such-berth.json"))
...
def test_request_identity_is_physics(root, triage_berth, hypothesize_berth):
    ...
    silent = good_packet(hypothesize_berth)
    silent.pop("ticket", None)
    silent["hypothesize_ref"] = foreign
    try:
        validate_validate(silent, root=root)
