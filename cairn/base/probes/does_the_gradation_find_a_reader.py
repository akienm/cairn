...
Berthed beside ``cairn/base`` because that is WHAT IT WATCHES: the nest primitive
lives at ``cairn/base/nest.py``, and the one way the ruling
...
address: cairn/base's own proof carries a toy second tenant importing
``cairn.base.nest`` — load-bearing there precisely BECAUSE it exercises the berth —
...
names ``cairn.base.nest`` a dozen times and carries it zero. The census parses
...
_EXCLUDED = (Path(__file__).resolve().parents[1],
             _REPO_ROOT / "cairn" / "build_inspector")


def _imports_the_berth(source: str) -> bool:
    """True iff the source IMPORTS cairn.base.nest — ``import cairn.base.nest``,
    ``from cairn.base.nest import X``, or ``from cairn.base import nest`` — read from
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            if any(a.name == "cairn.base.nest" for a in node.names):
                return True
        elif isinstance(node, ast.ImportFrom):
            if node.level:
                continue
            if node.module == "cairn.base.nest":
                return True
            if node.module == "cairn.base" and any(a.name == "nest" for a in node.names):
                return True
    return False
...
def survey_the_corpus() -> dict:
    ...
    against real second carriers of ``cairn.base.nest`` (importing .py files outside
